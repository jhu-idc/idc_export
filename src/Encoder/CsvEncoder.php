...
class CsvEncoder implements EncoderInterface, DecoderInterface {
  ...
  public function encode($data, $format, array $context = []) {
   \Drupal::logger("cvs_serial")->info("Encode received: " . print_r($data, true));
    switch (gettype($data)) {
      case "array":
        break;
    ...
    try {
      ...
      foreach ($data as $row) {
          \Drupal::logger("csv_serial")->info("row is: " . print_r($row, true));
          \Drupal::logger("csv_serial")->info("type is: " . print_r($row['type'], true));
          // is it a collection object? 
          if (str_contains($row['type'], "collection_object")) {
              \Drupal::logger("csv_serial")->info("HERE");
              $members = \Drupal::entityTypeManager()
                ->getListBuilder('node')
                ->getStorage()
                ->loadByProperties([
                    //'type.target_id' => 'islandora_object',
                    'field_member_of' => $row['nid']
                ]);
              \Drupal::logger("csv_serial")->info("PAST HERE, count is: " . count($members));
              foreach ($members as $node) {
                  $narray = $node->toArray();
                  \Drupal::logger("csv_serial")->info("entity as array: " . print_r($narray, true));
                  \Drupal::logger("csv_serial")->info("entity : " . print_r($node, true));
                  $csv->insertOne($node->toArray());
              }
          }
          $csv->insertOne($row);
      }
